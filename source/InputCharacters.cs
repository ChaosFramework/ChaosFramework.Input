...
{
    using Layouts;
    using KeyboardLayoutMap = SysCol.Dictionary<KeyboardLayouts, SysCol.Dictionary<Keyboard.HidUsage, InputCharacters.Axis>>;
    using MappedHidBaseLayoutMap = SysCol.Dictionary<System.Type, SysCol.Dictionary<object, InputCharacters.Axis>>;
    using MappedHidLayoutMap = SysCol.Dictionary<Layouts.HidLayout, SysCol.Dictionary<object, InputCharacters.Axis>>;
    using UnmappedHid = SysCol.Dictionary<uint, SysCol.Dictionary<uint, InputCharacters.Axis>>;
    ...
    public class InputCharacters
    {
        ...
        public InputCharacters(System.IO.Stream stream, LayoutManager layoutMgr)
        {
        ...
        void LoadKeyboard(Xml.XmlNode keyboardNode)
        {
            foreach (Xml.XmlNode layoutNode in keyboardNode.SelectNodes("Layout"))
            {
                ...
                if (attrName != null)
                {
                    ...
                    {
                        SysCol.Dictionary<Keyboard.HidUsage, Axis> dict;
                        if (!keyboardLayouts.TryGetValue(layout, out dict))
                            keyboardLayouts[layout] = dict = new SysCol.Dictionary<Keyboard.HidUsage, Axis>();
                        ...
                        if (attrBase != null)
                        {
                            ...
                            {
                                SysCol.Dictionary<Keyboard.HidUsage, Axis> baseDict;
                                if (keyboardLayouts.TryGetValue(baseLayout, out baseDict))
                                    foreach (SysCol.KeyValuePair<Keyboard.HidUsage, Axis> entry in baseDict)
                        ...
                        {
                            Axis axis = Axis.Parse(typeof(Keyboard.HidUsage), line);
                            if (axis != null)
                                dict[(Keyboard.HidUsage)axis.key] = axis;
                        }
                    }
                }
            }
        ...
        public UnicodeChars GetAxisChar(InputAxis axis, KeyboardLayouts keyboardLayout)
        {
            ...
            else if (axis is Mouse.Position)
        ...
        public UnicodeChars GetAxisChar(Keyboard.HidUsage key, KeyboardLayouts keyboardLayout)
        {
            SysCol.Dictionary<Keyboard.HidUsage, Axis> layout;
            if (keyboardLayouts.TryGetValue(keyboardLayout, out layout))
            {
                Axis character;
                if (layout.TryGetValue(key, out character))
        ...
        public UnicodeChars GetAxisChar(Keyboard.Key key, KeyboardLayouts keyboardLayout)
            => GetAxisChar(key.hidKey, keyboardLayout);
