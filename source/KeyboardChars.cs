...
{
    using Key = Keyboard.HidUsage;

    using CharacterMap = SysCol.Dictionary<KeyboardLayouts, SysCol.Dictionary<KeyboardChars.Modifier, SysCol.Dictionary<Keyboard.HidUsage, char>>>;
    using KeyMap = SysCol.Dictionary<KeyboardLayouts, SysCol.Dictionary<KeyboardChars.Modifier, Keyboard.HidUsage[]>>;
    ...
    public static class KeyboardChars
    {
        ...
        static KeyboardChars()
        {
            using (System.IO.MemoryStream str = new System.IO.MemoryStream(Properties.Resources.KeyboardLayouts))
            {
                ...
                foreach (XmlNode node in doc.SelectNodes("//Key"))
                {
                    ...
                    KeyboardLayouts layout = Enum<KeyboardLayouts>.Parse(node_layout.Attributes["id"].Value);
                    Modifier modifier = Enum<Modifier>.ParseFlags(node_modifier.Attributes["id"].Value);
                    if (!Enum<Key>.TryParseFlags(node.Attributes["key"].Value, out Key key))
                    {
                        error += node.Attributes["key"].Value + "\n";
                        continue;
                    }
                    char c = node.Attributes["char"].Value[0];

                    if (!chars.ContainsKey(layout))
                        chars[layout] = new SysCol.Dictionary<Modifier, SysCol.Dictionary<Key, char>>();

                    if (!chars[layout].ContainsKey(modifier))
                        chars[layout][modifier] = new SysCol.Dictionary<Key, char>();

                    chars[layout][modifier][key] = c;
                }

                if (error != "")
                    throw new System.Exception($"Failed to map the following keys:\n{error}");
            }
        ...
        public static Modifier GetCharModifier(Keyboard keyboard)
        {
            ...
            if (keyboard[Key.ShiftLeft].value > THRESHOLD || keyboard[Key.ShiftRight].value > THRESHOLD) mod |= Modifier.Shift;
            if (keyboard[Key.ControlLeft].value > THRESHOLD || keyboard[Key.ControlRight].value > THRESHOLD) mod |= Modifier.Ctrl;
            if (keyboard[Key.AltLeft].value > THRESHOLD) mod |= Modifier.Alt;
            if (keyboard[Key.AltRight].value > THRESHOLD) mod |= Modifier.AltGr;
            return mod;
        }
